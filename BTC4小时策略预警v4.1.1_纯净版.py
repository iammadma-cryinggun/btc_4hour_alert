...
- 手动平仓后，相同信号会使用混合策略（新止损+旧止盈）
- V4.0监控用于判断日线整体风险，辅助4H交易决策
        """)
...
下次信号将作为新趋势处理。
    ...
    def v4_get_realtime_data(self):
        ...
        try:
            ...
            data = response.json()[0]  # 取data[0]是昨天已收盘的日线
        ...
        try:
            ...
            headers = {'Authorization': f'Bearer {self.config.coinalyze_api_key}'}
            ...
            if response.status_code == 200:
                ...
                result['ls'] = self.v4_df['多空比(LS)'].dropna().tail(1).iloc[0]

        except Exception as e:
            logger.error(f"LS API错误: {e}")
            result['ls'] = self.v4_df['多空比(LS)'].dropna().tail(1).iloc[0]
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result['oi'] = self.v4_df['持仓量(OI-百万)'].dropna().tail(1).iloc[0]

        except Exception as e:
            logger.error(f"OI API错误: {e}")
            result['oi'] = self.v4_df['持仓量(OI-百万)'].dropna().tail(1).iloc[0]

        # 清算量（最新非零）
        liq_series = self.v4_df['清算量(美元)'].dropna()
    ...
    def v4_calculate_diagnosis(self, data):
        """计算V4.0诊断结果"""
        # 获取参考窗口
        ref_window = self.v4_df.tail(30).copy()
    ...
    def v4_format_diagnosis_report(self, diagnosis, timeframe):
        """格式化V4.0诊断报告"""
        # 状态判定
...
📡 数据源: {diagnosis['source']}
    ...
    def v4_log_query(self, query_type, diagnosis):
        """记录V4.0查询日志"""
        import csv
        from datetime import datetime

        try:
            log_file = 'btcv4_query_log.csv'
            ...
            with open(log_file, 'a', newline='', encoding='utf-8') as f:
                ...
                if not file_exists:
                    writer.writerow([
                        'timestamp', 'query_type', 'price', 'volume',
                        'ls_ratio', 'oi_million', 'liquidation_usd',
                        'ls_rank_pct', 'oi_rank_pct', 'vol_rank_pct', 'liq_rank_pct',
                        'crash_score', 'surge_score',
            ...
            logger.info(f"📝 V4.0查询已记录: {query_type}")

        except Exception as e:
            logger.error(f"V4.0日志记录失败: {e}")
